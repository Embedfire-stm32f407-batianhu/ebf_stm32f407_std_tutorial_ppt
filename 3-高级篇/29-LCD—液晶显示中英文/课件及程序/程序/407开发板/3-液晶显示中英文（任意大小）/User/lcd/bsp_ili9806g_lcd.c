									...
									uint8_t en_cn)		//0为英文，1为中文	
{
	uint8_t *pts,*ots;
	//根据源字模及目标字模大小，设定运算比例因子，左移16是为了把浮点运算转成定点运算
	float xrIntFloat_16=((float)in_width)/out_width; 
  float yrIntFloat_16=((float)in_heig)/out_heig;
    ...
    {
				float srcx_16=0;
        pSrcLine=pts+in_width*((uint16_t)srcy_16);				
        for (x=0;x<out_width;x++) /*行内像素遍历*/
        {
            ots[x]=pSrcLine[(uint16_t)srcx_16]; //把源字模数据复制到目标指针中
						
							if(y==0)
							printf("\r\n x = %d,srcx_16=%f,(uint16_t)srcx_16=%d",x,srcx_16,(uint16_t)srcx_16);
            srcx_16+=xrIntFloat_16;			//按比例偏移源像素点
        }
        srcy_16+=yrIntFloat_16;				  //按比例偏移源像素点
        ots+=out_width;						
